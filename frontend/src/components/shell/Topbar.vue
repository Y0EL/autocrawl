...
watch(debouncedTerm, async (term) => {
  ...
  try {
    const [vRes, eRes] = await Promise.all([
      api.vendors({ search: term, limit: 6 }).catch(() => ({ items: [] as Vendor[] } as { items: Vendor[] })),
      api.expos({ search: term, limit: 6 }).catch(() => ({ items: [] as Expo[] } as { items: Expo[] })),
    ])
    vendorResults.value = (vRes.items ?? []) as Vendor[]
  ...
  <header class="autocrawl-topbar rule-b bg-bg relative z-50 flex h-[64px] shrink-0 items-stretch">
    ...
    <div ref="searchWrap" class="flex flex-1 items-center px-5 relative min-w-0">
      <div class="flex w-full items-center gap-3">
        <FaIcon :icon="['fas', 'magnifying-glass']" class="text-[13px] text-ink-mute" />
        <input
          ref="searchInput"
          v-model="search"
          type="text"
          placeholder="Cari vendor, ekspo, industri, negara…"
        ...
        <div v-if="vendorResults.length > 0">
          ...
          >
            <div class="flex items-baseline justify-between gap-3">
              <span class="text-[14px] text-ink truncate">{{ v.company_name || v.domain || '(tanpa nama)' }}</span>
              <span class="num-display text-[11px] text-ink-mute tabular-nums shrink-0">{{ v.domain || '—' }}</span>
            </div>
            <div v-if="v.industries?.length || v.registrar_country" class="mt-0.5 flex items-baseline gap-2">
              <span v-if="v.registrar_country" class="label label-mute">{{ v.registrar_country }}</span>
              <span v-if="v.industries?.length" class="text-[12px] text-ink-2 truncate">{{ v.industries.slice(0, 2).join(' · ') }}</span>
            </div>
          </button>
        </div>

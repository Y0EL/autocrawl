...
type Tab = 'profil' | 'kontak' | 'katalog' | 'sumber' | 'json'
const tab = ref<Tab>('profil')
const TABS: Array<{ id: Tab; label: string }> = [
  { id: 'profil',  label: 'Profil' },
  { id: 'kontak',  label: 'Kontak' },
  { id: 'katalog', label: 'Katalog' },
  { id: 'sumber',  label: 'Provenance' },
  { id: 'json',    label: 'JSON' },
]
...
const confidencePct = computed(() => Math.round((data.value?.confidence_score ?? 0) * 100))
const confidenceTone = computed<'ok' | 'amber' | 'crit'>(() => {
  if (confidencePct.value >= 75) return 'ok'
  if (confidencePct.value >= 40) return 'amber'
  return 'crit'
})
...
<template>
  <div class="vd-canvas">
    ...
    <template v-else-if="data">
      ...
      <section class="vd-hero">
        ...
        <div class="vd-hero__layout">
          ...
          <div class="vd-identity fade-up" style="animation-delay: 80ms">
            <div class="vd-logo" :data-elite="confidencePct >= 90 ? 'true' : 'false'">
              <img
          ...
          <aside class="vd-margin fade-up" style="animation-delay: 160ms">
            ...
            <div class="vd-stat">
              ...
              <div
                class="vd-stat__num"
                :class="{
                  'is-ok':    confidenceTone === 'ok',
                  'is-amber': confidenceTone === 'amber',
                  'is-crit':  confidenceTone === 'crit',
                }"
              >
      ...
      <section v-if="tab === 'profil'" class="vd-folio">
        <article class="vd-folio__main">
          ...
          <div v-if="data.enrichment_gap.length" class="vd-gap">
            ...
            <p class="vd-gap__body">Field-field ini menanti enrichment berbayar.</p>
      ...
      <!-- SUMBER -->
      <section v-else-if="tab === 'sumber'" class="vd-sumber">
        <div class="bezel bezel-lg">
          <div class="bezel-core p-6">
            <span class="eyebrow eyebrow-accent">// SOURCE TRAIL</span>
            <HudProvenanceTimeline v-if="data.source_trail.length" :entries="data.source_trail" class="mt-4" />
            <div v-else class="vd-empty">
              <div class="vd-empty__glyph">
                <svg width="28" height="28" viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.5">
                  <circle cx="7" cy="7" r="5" /><circle cx="7" cy="7" r="1" fill="currentColor" />
                </svg>
              </div>
              <span class="label label-mute mt-3">Tidak ada provenance</span>
              <span class="text-xs text-ink-mute mt-1">Vendor ini belum punya jejak sumber tercatat.</span>
            </div>
          </div>
        </div>
      </section>

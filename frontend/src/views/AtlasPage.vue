...
const recents       = computed(() => recentVendors.data.value?.items ?? [])
        ...
        <div class="card p-5 h-full">
          ...
          <ul class="divide-y divide-[rgb(var(--rule)/var(--rule-alpha))]">
            ...
            >
              ...
              <span
                class="pill"
                :class="v.status === 'enriched' ? 'pill-ok'
                       : v.status === 'unresolved' ? 'pill-amber'
                       : (v.status === 'enrich_failed' || v.status === 'scope_rejected' || v.status === 'validation_rejected') ? 'pill-crit'
                       : ''"
              >
                {{ v.status }}
              </span>

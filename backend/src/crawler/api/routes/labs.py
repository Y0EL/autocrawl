...
async def list_candidates(
    ...
    async with get_session() as session:
        if only_with_email:
            rows = await fusion_repo.vendors_with_verified_email(
                session, limit=limit, offset=offset, search=search
            )
        else:
            stmt = select(VendorORM).order_by(VendorORM.confidence_score.desc()).limit(limit).offset(offset)
            if search:
                like = f"%{search.lower()}%"
...
async def create_fusion(req: FusionCreateRequest) -> FusionRead:
    ...
    svg_path = await render_composite(
        fusion_id,
        name=artifacts.name,
        tagline=artifacts.tagline,
        source_logos=source_logos,
    )
    image_url = f"/static/fusions/{svg_path.name}"

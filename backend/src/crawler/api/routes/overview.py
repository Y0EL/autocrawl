...
@router.get("/overview")
async def overview(session: AsyncSession = Depends(get_db)) -> dict:
    settings = get_settings()
    vendors_total = await vendor_repo.count(session)
    ...
    industry_breakdown = await vendor_repo.industry_breakdown(session)
    return {
...
async def stats_industries(session: AsyncSession = Depends(get_db)) -> list[dict]:
    return await vendor_repo.industry_breakdown(session)
...
async def stats_timeline(
    ...
    return await vendor_repo.timeline(session, days=days)

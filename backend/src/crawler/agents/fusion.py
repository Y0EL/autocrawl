...
async def suggest_fusions(candidates: list[Vendor], *, k: int = 5) -> list[FusionSuggestion]:
    ...
    try:
        ...
        result = await chat([sys, user], use_heavy=True, response_format=_SuggestionsWrap)
...
async def generate_artifacts(vendors: list[Vendor], hint: str | None = None) -> FusionArtifacts:
    ...
    try:
        ...
        result = await chat([sys, user], use_heavy=True, response_format=FusionArtifacts)
        _log.info(
            "fusion.artifacts_ok",
            name=result.name,
            duration_s=round(time.monotonic() - started, 2),
...
async def draft_email(
    *,
    ...
    try:
        ...
        result = await chat([sys, user], use_heavy=True, response_format=EmailDraftPayload)

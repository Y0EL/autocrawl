...
async def _generate_product_brief(
    ...
    *,
    ...
    try:
        result = await chat(
            [system, user], tier="light", response_format=_ProductBrief
        )
...
async def enrich_vendor_products(
    ...
    *,
    ...
    avoids runaway cost for vendors with 30+ products. Top items kept
    are simply the first N (listing extractor preserves order seen on
    expo page).
    """
    ...
    products_input = products_input[:product_cap]
